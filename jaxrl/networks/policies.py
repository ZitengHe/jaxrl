...
class NormalTanhAttentionPolicy(nn.Module):
    hidden_dims: Sequence[int]
    action_dim: int
    state_dependent_std: bool = True
    dropout_rate: Optional[float] = None
    ...
    def __call__(self,
        ...
        z_action = CBAM(name='action_cbam', reduction_ratio=self.reduction_ratio)(outputs)

        means = nn.Dense(self.action_dim,
                         kernel_init=default_init(
                             self.final_fc_init_scale))(z_action)
        if self.init_mean is not None:
            means += self.init_mean
    ...
    def __call__(self,
        ...
        z = MLP(hidden_dims=self.share_hidden_dims,
                ...
                layer_norm=self.layer_norm)(observations,
                                                training=training)
        outputs_action = MLP(self.action_hidden_dims, 
                           activations=self.activations,
                           dropout_rate=self.dropout_rate, 
        ...
        if self.tanh_squash_distribution:
            return tfd.TransformedDistribution(distribution=base_dist_action, bijector=tfb.Tanh()), base_dist_state
...
class TwoHeadsAttentionGaussianPolicy(nn.Module):
    share_hidden_dims: Sequence[int]
    action_hidden_dims: Sequence[int]
    state_hidden_dims: Sequence[int]
    state_dim: int
    ...
    def __call__(self,
        ...
        # Apply CBAM
        z_action = CBAM(name='action_cbam', reduction_ratio=self.reduction_ratio)(z)
        z_state = CBAM(name='state_cbam', reduction_ratio=self.reduction_ratio)(z)
        outputs_action = MLP(self.action_hidden_dims, 
                           activations=self.activations,
                           dropout_rate=self.dropout_rate, 
                           layer_norm=self.layer_norm)(z_action)
        means_action = nn.Dense(self.action_dim,
                                kernel_init=default_init(
                                self.log_std_scale
                                ))(outputs_action)
        outputs_state = MLP(self.state_hidden_dims, 
                          activations=self.activations, 
                          dropout_rate=self.dropout_rate, 
                          layer_norm=self.layer_norm)(z_state)
        means_state = nn.Dense(self.state_dim,
                               kernel_init=default_init(
                               self.log_std_scale
                                ))(outputs_state)
...
#                                                 training=training)
#         # Apply CBAM
#         z_action = CBAM(name='action_cbam')(z)
#         z_state = CBAM(name='state_cbam')(z)
...
def _sample_next_states(
    ...
    if distribution == 'det':
        ...
        dist_a, dist_s = actor_apply_fn({'params': actor_params}, observations,
                            temperature)
        rng, key = jax.random.split(rng)
        return rng, dist_s.sample(seed=key)
...
def sample_next_states(
        ...
        temperature: float = 1.0,
        distribution: str = 'log_prob') -> Tuple[PRNGKey, jnp.ndarray]:
    return _sample_next_states(rng, actor_apply_fn, actor_params, observations,
                           temperature, distribution)